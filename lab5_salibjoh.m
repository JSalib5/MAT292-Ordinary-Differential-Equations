...
%b) 100% of solutions in the domain show it decays while oscillating
%c) The exact solution: y(t) = exp(-t/2) * [c1*cos(2t)+c2*sin(2t)] shows
%that there are cosine and sine summed components that results in the
%oscillating motion shown in the solution. Further more the negative
%component ahead of t in the exponential results in the decaying motion
%observed.
%% Exercise 2
% Consider the ODE:
%%
...
%b) 100% of solutions in the domain show it grows
%c) The exact solution: y(t) = c1*exp((2-sqrt(3))*t/2)+c2*exp((2+sqrt(3))*t/2) 
%shows that there are no osscillating functions that results in no
%oscillating motion in the solution. Further more the positive
%components ahead of t in the exponentials results in the growing motion
%observed.
%% Exercise 3
% Consider the ODE:
%%
...
%b) 100% of solutions in the domain show it decays
%c) The exact solution: y(t) = c1*exp((sqrt(2)-sqrt(3))*t/2)+c2*exp((-sqrt(2)-sqrt(3))*t/2) 
%shows that there are no osscillating functions that results in no
%oscillating motion in the solution. Further more the negative
%components ahead of t in the exponentials results in the decaying motion
%observed.
%% Example
% Consider the ODE:
%%
...
%a)
chareqn = [1 2 6 2 5];
eigenvalues = roots(chareqn)
%hence the 4 eigen values are 2 sets of conjugates -1 +- 2i, +-i. Hence the
%solution will be the sum of the general solution of for each conjugate
%pair hence, y(t) = exp(-t)*(c1*sin(2t) + c2*cos(2t)) + c3*sin(t) +
%c4*cos(t). 

%b) 50% of solutions would will decay while oscilating when both c4 and c3
%are equal to 0. The remaining solutions just oscillate as, any values in the 
%exponential decay will result in 0 added to a cosine and sine functions as
%t goes to inf. 
%% Exercise 5
...
%a) grows for all solutions as both exponentials have positive coefficients
%b) grows and decays for each half of solutions as both exponentials have 
% positive and negative coefficients
%c) decays for all solutions as both exponentials have negative coefficients 
%d) decays while oscillating for all solutions as there are sine, cosine
%components multiplied with a negative cofficient exponential 
%e) just oscillates as there is no exponential component multiplied with
%the cosine, sine components
%f) grows while oscillating for all solutions as there are sine, cosine
%components multiplied with a positive cofficient exponential 
...
f = @(t,y,dy) -exp(-t/5)*dy - (1-exp(-t/5))*y + sin(2*t); %defining second order ODE
y0 = 1; %defining initial conditions
y1 = 0;
t0 = 0;
tN = 20;
h = 0.01;

[tDE2, yDE2] = DE2(f,t0,tN,y0,y1,h); %using the new function solver

%using plot(tDE2, yDE2) in command window to place in iode

%the approximation maintains high accuracy along majority of the solution
%only deviating slightly at minima and maxima.