...
N = 101; %defining size of x vector
h = (10)/(N-1); %defining distance between points (-3 and 7) within vector x
x = zeros(N, 1); %defining vector x
y = zeros(N, 1); %defining vector y

for j = 1:N
    x(j) = (j-1)*h - 3; %assigning values to vector x
    y(j) = abs(x(j))*x(j); %assigning values to vector y
end

plot(x,y); %plotting both vectors
...
N = 5000001; %defining size for x and y vectors
h = 5/(N-1); %defining spacing within x vector
x = zeros(N,1); %defining x vector

clear y; %clearing y

y = zeros(N,1); %defining y vector

for j = 1:N
    x(j) = (j-1)*h - 2; %assigning x values 
end

tic; %timing y assignment using loop
for i = 1:N
    y(i) = atan(x(i)); %first loop for y vector assignment
end
toc;

clear y; %clearing y vector prior to vectorization method

tic; %timing y assignment using vectorization
y = atan(x);
toc;

%it is visible that vectorization is significantly faster
...
%General ODE solution: y(t) = t*exp(-t^2) + C*exp(-t^2)
%Particular Solutions:
% 1: y1(t) = exp(-t^2)[t-1-e]
% 2: y2(t) = exp(-t^2)[t-1]
% 3: y3(t) = exp(-t^2)[t-1+e]

t = linspace(1, 5, 100); %assigning independent variable vector in range [1, 10] with 100 points
y1 = exp(-t.^2).*(t-1-exp(1)); %assigning solution 1 vector
y2 = exp(-t.^2).*(t-1); %assigning solution 2 vector
y3 = exp(-t.^2).*(t-1+exp(1)); %assigning solution 3 vector

plot(t,y1,t,y2,t,y3) %plotting all 3 solution vectors with respect to t
xlabel('t');
ylabel('y(t) solutions');
title('Plot of 3 particular solutions of dy/dt + (2*t)*y = 1/exp(t^2) with respect to t');
legend('exp(-t^2)*(t-1-exp(1))', 'exp(-t^2)*(t-1)', 'exp(-t^2)*(t-1+exp(1))','Location','SouthEast');
...
f = @(x) (x.^3 - abs(x)).*log(x.^2+1); %assigning function f(x)

disp(f([0,1,-1])) %computing f(0), f(1), f(-1)

x = linspace(-5, 5, 100); %defining x vector
plot(x,f(x)) %plotting f(x) with respect to x
xlabel('x');
ylabel('f(x)');
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4; %defining f(x,y)
disp(f(0,0))
disp(f(pi/2,1))
disp(f(1,-1)) %computing f(0,0), f(pi/2,1), f(1,-1)
...
g = @(y) f(x,y);
y = fzero(g, guess);
fprintf(' y(%g) = %g\n', x, y); %finding the y value solution at x = 0

y = 1/2;
g = @(x) f(x,y);
x = fzero(g, guess);
fprintf(' y(%g) = %g\n', x, y); %finding solution 1 at y = 1/2 for x

y = 1/2;
guess = 0.5;
x = fzero(g, guess);
fprintf(' y(%g) = %g\n', x, y); %finding solution 2 at y = 1/2 for x
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4; %defining f(x,y)
xvals = linspace(-2, 2, 100); % defining x in range [-2, 2] with 100 points
yvals = zeros(size(xvals)); % defining space for y

for j = 1:length(xvals)
   
   yvals(j) = fzero(@(y) f(xvals(j), y), 0); %loop through all values
...
%general implicit solution to ODE: ln(y) + sin(x) - x^3 - C = 0
%particular solution at y(0) = 1: ln(y) + sin(x) - x^3 = 0

f = @(x,y) log(y) + sin(x) - x.^3; %defining f(x,y)
xvals = linspace(-1.5, 1.25, 100); % defining x in range [-1.5, 1.25] with 100 points
yvals = zeros(size(xvals)); % defining space for y

for j = 1:length(xvals)
   
   yvals(j) = fzero(@(y) f(xvals(j), y), 1); %loop through all values
...
%As x approaches infinity the direction field solutions become more
%vertical as the slope approaches infinity, the directions become more
%perpendicular to the x axis. There is only one horizontal solution y = 0
%that does not diverge. Solutions are symmetrical about the y-axis as
%the slope is proportional to the square of y hence negative and positive
%values along the x axis will result in the same y value, hence providing
%the symmetry along the y axis. (-x)^2 = x^2
%% Exercise 12
...
%The list of solutions that appear as x approaches infinity are y = 2, 1,-1
%Solutions between -1 <y_0< 2 tend to converge to y = 1 showing that this is a stable
%equilibrium point. Whereas at y = 2, solutions quickly diverging to y = 1
%and infinity, this shows that y = 2 is an unstable equilibrium point. While,
%y_0 > -1 solutions converge to y = 1, when y_0 < -1 solutions converge at 
% y = -1 making it a semi-stable equilibrium point.
